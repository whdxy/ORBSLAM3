 ...
 * @brief 预积分计算，更新noise
 *
 * @param[in] acceleration  加速度(两IMU帧间平均值)
 * @param[in] angVel        角速度(两IMU帧间平均值)
 * @param[in] dt            两IMU帧之间时间差
 */
void Preintegrated::IntegrateNewMeasurement(const Eigen::Vector3f &acceleration, const Eigen::Vector3f &angVel, const float &dt)
{
    ...
    // 因为随着时间推移，不可能每次都重新计算雅克比矩阵，所以需要做J(k+1) = j(k) + (~)这类事，分解方式与AB矩阵相
    // 论文作者对forster论文公式的基础上做了变形，然后递归更新，参见 https://github.com/UZ-SLAMLab/ORB_SLAM3/issues/212
    /// 上述的说法不对，下面的公式来源于论文的 eq.(67)(68)(69) 并将其改写成递推形式，以方便计算
    /// 在此处仅更新了JPa，JPg，JVa，JVg，而将 JRg 放到了下面去更新，这是因为前四者的更新依赖 JRg
    ...
    IntegratedRotation dRi(angVel,b,dt); /// 计算i->i+1旋转增量
    ...
    // Update covariance
    /// Nga 和 NgaWalk 都是固定值(6*6对角矩阵)
    /// 20230706疑问1：这里的C是协方差矩阵，还是信息矩阵？
    /// 20230706疑问2：C的从9*9开始的6*6矩阵是用来干啥的
    C.block<9,9>(0,0) = A * C.block<9,9>(0,0) * A.transpose() + B*Nga*B.transpose();
    C.block<6,6>(9,9) += NgaWalk;
    //std::cout << "=========================" << std::endl;
    //std::cout << Nga.diagonal() << std::endl << std::endl;
    //std::cout << NgaWalk.diagonal() << std::endl;
    //std::cout << "=========================" << std::endl;
...
{
    std::unique_lock<std::mutex> lock(mMutex);
    //std::cout << "dv1:" << dV << std::endl;
    //std::cout << "dv2:" << dV + JVg * db.head(3) + JVa * db.tail(3) << std::endl;
