...
void Optimizer::LocalInertialBA(KeyFrame *pKF, bool *pbStopFlag, Map *pMap, int& num_fixedKF, int& num_OptKF, int& num_MPs, int& num_edges, bool bLarge, bool bRecInit)
{
    ...
    //cout << "lpOptVisKFs'size:" <<  lpOptVisKFs.size() << endl;
    /// 下面这个也没什么用处
    ...
    for(size_t i=0, iend = vpEdgesMono.size(); i<iend; i++)
    {
        ...
        e->computeError();
        const float chi2 = e->chi2(); // chi2 = error*information*error
        e_points += chi2;
        //cout << "vpEdgesMono chi2:" << e->chi2() << endl;
    }
    cout << "e_points1:" << e_points << "  " << e_points/vpEdgesMono.size() << endl;
    double e_ves = 0;
    for(size_t i=0, iend = vei.size(); i<iend; i++)
    {
        EdgeInertial* e = vei[i];
        e->computeError();
        const float chi2 = e->chi2(); // chi2 = error*information*error
        e_ves += chi2;
        //cout << "vei chi2:" << e->chi2() << endl;
    }
    cout << "e_ves2:" << e_ves << "  " << e_ves/vei.size() << endl;
    //cout << "------------------------------------" << endl;
     */
    ...
    cout << "e_points2:" << e_points << "  " << e_points/vpEdgesMono.size() << endl;
