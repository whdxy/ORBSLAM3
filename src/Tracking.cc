...
void Tracking::PreintegrateIMU()
{
    ...
    while(true)
    {
        ...
        bool bSleep = false;
        {
            ...
            if(!mlQueueImuData.empty())
            {
                ...
                // 同样最后一个的imu数据时间戳也不能比当前帧时间间隔晚0.001
    ...
    //cout << "mLastFrame id:" << mLastFrame.mnId << endl << mLastFrame.mImuBias << endl;
...
void Tracking::Track()
{
    ...
    if ((mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD) && mpLastKeyFrame){
        ...
        //cout << "id: " << mpLastKeyFrame->mnId << "  " << mpLastKeyFrame->GetImuBias() << endl;
    }
...
bool Tracking::TrackWithMotionModel()
{
    ...
    if (mpAtlas->isImuInitialized() && (mCurrentFrame.mnId>mnLastRelocFrameId+mnFramesToResetIMU))
    {
        ...
        // IMU完成初始化 并且 距离重定位挺久不需要重置IMU，直接用IMU来估计位姿，就不需要后面的恒速模型追踪
        PredictStateIMU(); //
        return true;
    }
    else
